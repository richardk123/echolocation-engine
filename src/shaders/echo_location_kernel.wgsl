...
fn main(@builtin(global_invocation_id) id : vec3<u32>) 
{
    ...
    textureStore(color_buffer, p1_screen_pos, vec4<f32>(f32(scene.listenerPos.x), f32(scene.listenerPos.y), 1.0, 1.0));
    textureStore(color_buffer, p2_screen_pos, vec4<f32>(f32(scene.listenerPos.x), f32(scene.listenerPos.y), 1.0, 1.0));
}
